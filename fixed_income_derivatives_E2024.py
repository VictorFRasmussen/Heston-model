...
def simul_ho_lee(r0,theta,sigma,N,M,T,method = "euler"):
    r = np.zeros([N,M+1])
    r[0] = r0
    delta = T/M
    ...
    if method == "euler":
        ...
        for n in range(0,N):
            for m in range(1,M+1):
                r[n,m] = r[n,m-1] + theta[m]*delta + sigma*delta_sqrt*Z[n,m-1]
...
def zcb_curve_fit(data_input,interpolation_options = {"method": "linear"},scaling = 1):
    ...
    T_fra_fit.sort()
    R_fra_fit, R_fra_fit_deriv = interpolate(T_swap_curve,R_swap_curve,T_fra_fit,interpolation_options)
...
def zcb_curve_interpolate(T,R,interpolation_options = {"method":"linear"},resolution = 1):
    T_inter = np.array([i*(1/(resolution*12)) for i in range(1,int(T[-1])*12*resolution + 1)])
    N = len(T_inter)
    p_inter = np.ones([N])
    R_inter = np.zeros([N])
    f_inter = np.zeros([N])
    ...
    elif interpolation_options["method"] == "hermite" or interpolation_options["method"] == "nelson_siegel":
        R_inter, R_inter_deriv = interpolate(T,R,T_inter,interpolation_options = interpolation_options)
        for i in range(0,N):
            f_inter[i] = R_inter[i] + R_inter_deriv[i]*T_inter[i]
            p_inter[i] = np.exp(-R_inter[i]*T_inter[i])
    return p_inter, R_inter, f_inter, T_inter

def extrapolate(x,y,x_extra,extrapolation_options = {"method":"linear"}):
...
def interpolate(x,y,x_inter,interpolation_options = {"method":"linear", "transition": None}):
    N = len(x_inter)
    ...
    if interpolation_options["method"] == "linear":
        ...
        l, r = int(interpolation_options["degree"]/2),int((interpolation_options["degree"]+1)/2)
        M = len(x)
        coefs, degrees = np.zeros([M,r+l+1]), np.zeros([M],dtype = int)
        for i in range(0,M):
            idx_begin, idx_end = max(i-l,0), min(i+r+1,M)
            degrees[i] = idx_end - idx_begin - 1
            coefs[i,0:(degrees[i]+1)] = hermfit(x[idx_begin:idx_end],y[idx_begin:idx_end],degrees[i])
        y_inter[indices[0]], y_inter_deriv[0] = y[0], hermval(x_inter[0],hermder(coefs[0,0:degrees[0]+1]))
        if interpolation_options["transition"] == "smooth":
            for i in range(1,M):
                y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], hermval(x_inter[indices[i]],hermder(coefs[i,0:degrees[i]+1]))
                for idx in range(indices[i-1]+1,indices[i]):
                    w_right = (x_inter[idx] - x_inter[indices[i-1]])/(x_inter[indices[i]] - x_inter[indices[i-1]])
                    w_left = 1 - w_right
                    y_inter[idx] = w_left*hermval(x_inter[idx],coefs[i-1,0:degrees[i-1]+1]) + w_right*hermval(x_inter[idx],coefs[i,0:degrees[i]+1])
                    y_inter_deriv[idx] = w_left*hermval(x_inter[idx],hermder(coefs[i-1,0:degrees[i-1]+1])) + w_right*hermval(x_inter[idx],hermder(coefs[i,0:degrees[i]+1]))
        else:
            for i in range(1,M):
                y_inter[indices[i]] = y[i]
                if i < M-1:
                    y_inter_deriv[indices[i]] = 0.5*hermval(x_inter[indices[i]],hermder(coefs[i-1,0:degrees[i-1]+1])) + 0.5*hermval(x_inter[indices[i]],hermder(coefs[i,0:degrees[i]+1]))
                elif i == M-1:
                    y_inter_deriv[indices[i]] = hermval(x_inter[indices[i]],hermder(coefs[i-1,0:degrees[i-1]+1]))
                for idx in range(indices[i-1]+1,indices[i]):
                    y_inter[idx] = hermval(x_inter[idx],coefs[i-1,0:degrees[i-1]+1])
                    y_inter_deriv[idx] = hermval(x_inter[idx],hermder(coefs[i-1,0:degrees[i-1]+1]))
        ...
        l, r = 1, 1
        M = len(x)
        coefs = np.zeros([M,r+l+1])
        i = 0
        idx_begin, idx_end = i, i+l+r+1
        if (x[idx_begin] < x[idx_begin+1] and x[idx_begin+1] > x[idx_begin + 2]) or (x[idx_begin] > x[idx_begin+1] and x[idx_begin+1] < x[idx_begin+2]):
            coefs[i,0:3] = hermfit(x[idx_begin:idx_end],y[idx_begin:idx_end],2)
        else:
            x1, x2 = x[idx_begin+1]-x[idx_begin], x[idx_begin+2]-x[idx_begin]
            coefs[i,0:3] = nelson_siegel_coef(x1,x2,y[idx_begin],y[idx_begin+1],y[idx_begin+2])
        for i in range(1,M-1):
            idx_begin, idx_end = i-l, i+r+1
            if (x[idx_begin] < x[idx_begin+1] and x[idx_begin+1] > x[idx_begin + 2]) or (x[idx_begin] > x[idx_begin+1] and x[idx_begin+1] < x[idx_begin+2]):
                coefs[i,0:3] = hermfit(x[idx_begin:idx_end],y[idx_begin:idx_end],2)
            else:
                x1, x2 = x[idx_begin+1]-x[idx_begin], x[idx_begin+2]-x[idx_begin]
        ...
        if interpolation_options["transition"] == "smooth":
            for i in range(1,M):
                if i == M-1:
                    idx_begin, idx_end = i-2, i+r
                else:
                    idx_begin, idx_end = i-l, i+r+1
                if (x[idx_begin] < x[idx_begin+1] and x[idx_begin+1] > x[idx_begin + 2]) or (x[idx_begin] > x[idx_begin+1] and x[idx_begin+1] < x[idx_begin+2]):
                    if i == 1:
                        y_inter[indices[i-1]], y_inter_deriv[i-1] = y[0], hermval(x_inter[0],hermder(coefs[i,0:3]))
                        for idx in range(indices[i-1]+1,indices[i]):
                            y_inter[idx] = hermval(x_inter[idx],coefs[i,0:3])
                            y_inter_deriv[idx] = hermval(x_inter[idx],hermder(coefs[i,0:3]))
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], hermval(x_inter[indices[i]],hermder(coefs[i,0:3]))
                    elif i == M - 1:
                        for idx in range(indices[i-1]+1,indices[i]):
                            y_inter[idx] = hermval(x_inter[idx],coefs[i-1,0:3])
                            y_inter_deriv[idx] = hermval(x_inter[idx],hermder(coefs[i-1,0:3]))
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], hermval(x_inter[indices[i]],hermder(coefs[i,0:3]))
                    else:
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], hermval(x_inter[indices[i]],hermder(coefs[i,0:3]))
                        for idx in range(indices[i-1]+1,indices[i]):
                            w_right = (x_inter[idx] - x_inter[indices[i-1]])/(x_inter[indices[i]] - x_inter[indices[i-1]])
                            w_left = 1 - w_right
                            y_inter[idx] = w_left*hermval(x_inter[idx],coefs[i-1,0:3]) + w_right*hermval(x_inter[idx],coefs[i,0:3])
                            y_inter_deriv[idx] = w_left*hermval(x_inter[idx],hermder(coefs[i-1,0:3])) + w_right*hermval(x_inter[idx],hermder(coefs[i,0:3]))
                else:
                    if i == 1:
                        y_inter[indices[i-1]], y_inter_deriv[i-1] = y[0], -coefs[i-1,1]*coefs[i-1,2]
                        for idx in range(indices[i-1]+1,indices[i]):
                            y_inter[idx], y_inter_deriv[idx] = coefs[i-1,0] + coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx] - x_inter[indices[i-1]])), -coefs[i-1,1]*coefs[i-1,2]*np.exp(-coefs[i-1,2]*(x_inter[idx] - x_inter[indices[i-1]]))
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], -coefs[i,2]*coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[indices[i]] - x_inter[indices[i-1]]))
                    elif i == M - 1:
                        for idx in range(indices[i-1]+1,indices[i]):
                            y_inter[idx], y_inter_deriv[idx] = coefs[i-1,0] + coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx] - x_inter[indices[i-2]])), -coefs[i-1,2]*coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx] - x_inter[indices[i-2]]))
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], -coefs[i-1,2]*coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[indices[i]] - x_inter[indices[i-2]]))
                    else:
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], -coefs[i,0]*coefs[i,1]*np.exp(-coefs[i,2]*x1)
                        for idx in range(indices[i-1]+1,indices[i]):
                            w_right = (x_inter[idx] - x_inter[indices[i-1]])/(x_inter[indices[i]] - x_inter[indices[i-1]])
                            w_left = 1 - w_right
                            y_inter[idx] = w_left*(coefs[i-1,0]+coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx]-x_inter[indices[i-2]]))) + w_right*(coefs[i,0]+coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[idx]-x_inter[indices[i-1]])))
                            y_inter_deriv[idx] = w_left*(-coefs[i-1,2]*coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx]-x_inter[indices[i-2]]))) + w_right*(-coefs[i,2]*coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[idx]-x_inter[indices[i-1]])))
                        y_inter[indices[i]] = coefs[i,0] + coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[indices[i]]-x_inter[indices[i-1]]))
                        y_inter_deriv[indices[i]] = -coefs[i,2]*coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[indices[i]]-x_inter[indices[i-1]]))
        else:
            for i in range(1,M):
                if i == M-1:
                    idx_begin, idx_end = i-2, i+r
                ...
                if (x[idx_begin] < x[idx_begin+1] and x[idx_begin+1] > x[idx_begin + 2]) or (x[idx_begin] > x[idx_begin+1] and x[idx_begin+1] < x[idx_begin+2]):
                    if i == 1:
                        ...
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], hermval(x_inter[indices[i]],hermder(coefs[i,0:3]))
                        for idx in range(indices[i-1]+1,indices[i]):
                            y_inter[idx] = hermval(x_inter[idx],coefs[i-1,0:3]) + w_right*hermval(x_inter[idx],coefs[i,0:3])
                            y_inter_deriv[idx] = hermval(x_inter[idx],hermder(coefs[i-1,0:3])) + w_right*hermval(x_inter[idx],hermder(coefs[i,0:3]))
                else:
                    if i == 1:
                        y_inter[indices[i-1]], y_inter_deriv[i-1] = y[0], -coefs[i,0]*coefs[i,1]
                        for idx in range(indices[i-1]+1,indices[i]):
                            y_inter[idx] = coefs[i,0] + coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[idx] - x_inter[indices[i-1]]))
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], -coefs[i,2]*coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[indices[i]] - x_inter[indices[i-1]]))
                    elif i == M - 1:
                        for idx in range(indices[i-1]+1,indices[i]):
                            y_inter[idx], y_inter_deriv[idx] = coefs[i-1,0] + coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx] - x_inter[indices[i-2]])), -coefs[i-1,0]*coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx] - x_inter[indices[i-2]]))
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], -coefs[i-1,2]*coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[indices[i]] - x_inter[indices[i-2]]))
                    else:
                        y_inter[indices[i]], y_inter_deriv[indices[i]] = y[i], -coefs[i,0]*coefs[i,1]*np.exp(-coefs[i,2]*x1)
                        for idx in range(indices[i-1]+1,indices[i]):
                            y_inter[idx] = coefs[i-1,0]+coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx]-x_inter[indices[i-2]]))
                            y_inter_deriv[idx] = -coefs[i-1,2]*coefs[i-1,1]*np.exp(-coefs[i-1,2]*(x_inter[idx]-x_inter[indices[i-2]]))
                        y_inter[indices[i]] = coefs[i,0] + coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[indices[i]]-x_inter[indices[i-1]]))
                        y_inter_deriv[indices[i]] = -coefs[i,2]*coefs[i,1]*np.exp(-coefs[i,2]*(x_inter[indices[i]]-x_inter[indices[i-1]]))
...
def zcb_curve_swap_fit_obj(R_knot,T_known,T_knot,T_all,R_known,swap_data,interpolation_options,scaling = 1):
    sse = 0
    R_knot = list(R_knot)
    R_all, R_deriv = interpolate(T_known + T_knot,R_known + R_knot,T_all,interpolation_options)
